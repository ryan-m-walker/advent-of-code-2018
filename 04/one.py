...
def main(input_data):
    ...
    sorted_data = sorted(input_data, key=lambda x: str(re.search(
        r"(?<=\[).*(?=\])", x)))  # use a regex to grab the date out of the string

    # process the data using the process_data function
    guards = process_data(sorted_data)

    # use the max function to get the guard who has slept the most,
    # using the total_minutes_slept property as the key for sorting
    sleepy_boi = max(guards.values(), key=lambda boi: boi.total_minutes_slept)
    # once the guard who slept the most has been found,
    # use the max function with the enumerate function
    # to find the index of the slot with the highest count
    # in the guard's time_table. This index is equal to the
    # minute in the midnight hour that they sleep the most
    sleepy_time, _ = max(enumerate(sleepy_boi.time_table),
                         key=operator.itemgetter(1))

    # return both the guard's id and the minute
